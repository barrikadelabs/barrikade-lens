...
Managed Clerk deployments may separately enable privacy-minimized activation analytics. That dataset uses HMAC-derived user and workspace pseudonyms and excludes discovery data, evidence, names, email addresses, URLs, commands, and infrastructure identifiers. It is disabled in development, OIDC, and self-hosted modes. See [Managed Lens activation analytics](posthog-analytics.md) for the exact contract, consent behavior, delivery controls, and erasure procedure.
