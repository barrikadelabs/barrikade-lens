...
Managed Clerk deployments may separately enable privacy-minimized product analytics. That dataset uses HMAC-derived user and workspace pseudonyms and excludes discovery data, evidence, names, email addresses, URLs, commands, and infrastructure identifiers. Maximally masked replay, numeric web performance, sanitized exception classes, rating-only survey answers, and feature-flag exposures may be enabled under the same account preference. It is disabled in development, OIDC, and self-hosted modes. See [Managed Lens product analytics](posthog-analytics.md) for the exact contract, consent behavior, delivery controls, and erasure procedure.
