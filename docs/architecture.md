...
Detection separates facts that are often confused. Repository instructions are repository context, framework dependencies are framework use, and supporting runtimes remain supporting runtimes. Explicit agent definitions create inventory entities, but definitions bundled beneath an installed runtime are subordinate components rather than independently deployed autonomous root systems. Kubernetes workloads using Node.js or Python do not become agents by association. Unknown future runtimes can still be discovered through explicit workload labels, A2A/MCP documents, custom-agent descriptors, and open Agent Skills without waiting for a named product signature.
