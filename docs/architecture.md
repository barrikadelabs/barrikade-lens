...
Entity IDs are deterministic and organization-scoped. Cross-surface keys use repository URLs and commits, image digests, Kubernetes UIDs, configuration fingerprints, and explicit labels. PostgreSQL stores current inventory separately from source presence and 90-day evidence/change history.
