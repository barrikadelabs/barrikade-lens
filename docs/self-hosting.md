...
The Hub accepts Snapshot 1.1 and 1.2 collectors while current collectors emit Snapshot 1.3. Version 1.2 added cloud targets, cloud environments, workload identities, knowledge stores, containment, and run-as relationships. Version 1.3 adds explicit relationship surface, observation time, and declared/discovered/observed state. A 1.0 snapshot receives HTTP 426 with an explicit upgrade error. A legacy database migration creates one `legacy_identity` target for each existing source and never merges targets by hostname. For a pre-release local reset, stop Compose with volumes, rebuild, generate a new enrollment code, and re-enroll so the endpoint creates its persistent installation identity.
