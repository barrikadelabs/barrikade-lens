...
export function platformPackage(platform = process.platform, architecture = process.arch): string {
...
export function binaryPath(platform = process.platform, architecture = process.arch): string {
  ...
  try {
    ...
    throw new Error(
      `The optional native package ${packageName} is missing. Reinstall without disabling optional dependencies. ` +
