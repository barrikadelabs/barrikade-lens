...
export function FindingsPage({ api, revision }: { api: API; revision: number }) {
  ...
  return <div className="page-stack">
    ...
    <section className="panel data-panel"><PanelHeading title="What needs review" detail="Potential issues across your organization, ordered by impact and how recently Lens observed them." count={items.length} /><div className="finding-list">{items.map((finding) => <button key={finding.id} className="finding-row" onClick={() => navigate(`/findings/${encodeURIComponent(finding.id)}?${search.toString()}`)}><span className={`severity-pill ${finding.severity}`}>{finding.severity}</span><span><b>{finding.title}</b><small>{finding.root_name} · last supported {finding.evidence_last_seen_at ? relative(finding.evidence_last_seen_at) : relative(finding.last_seen_at)}</small></span><span className={finding.effective_ownership?.owned ? "fact good" : "fact quiet"}>{finding.effective_ownership?.owner_name || "No owner assigned"}</span><ChevronRight size={15} /></button>)}{!items.length && <Empty icon={CheckCircle2} title="No findings match" detail="Lens found no current issues for these filters." />}</div>{remote.data?.next_cursor && <button className="load-more" onClick={() => { captureAnalytics({ name: "lens_interaction", properties: { surface: "findings", interaction: "load_more" } }); setCursor(remote.data!.next_cursor!); }}>Load more findings <ChevronDown size={15} /></button>}</section>
