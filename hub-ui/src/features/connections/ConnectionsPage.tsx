...
function EnvironmentsPage({ api, revision, onResults, startWizard = false }: { api: API; revision: number; onResults: () => void; startWizard?: boolean }) {
  ...
  return <div className="page-stack environments-page">
    ...
    {wizard && canManage && <div className="modal-overlay environment-wizard-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) reset(); }}><section className="environment-wizard" role="dialog" aria-modal="true" aria-label="Scan environment"><button className="drawer-close" aria-label="Close scan setup" onClick={reset}><X size={18} /></button><header><p className="eyebrow">SCAN ENVIRONMENT</p><h2>{setup ? "Complete source setup" : kind === "endpoint" && !endpointMode ? "Choose how this device reports" : kind === "endpoint" && endpointMode === "continuous" && !deploymentMethod ? "How do you want to deploy?" : kind ? `Connect ${selected?.title}` : "Let’s map your AI environment"}</h2><p>{setup ? "The generated setup is least-privilege and expires shortly." : kind === "endpoint" && !endpointMode ? "Get one-time results now, or keep the device inventory current automatically." : kind === "endpoint" ? "Choose a platform for the generated command." : "Connect the places where AI might be running."}</p></header>
      ...
      {kind === "github_repository" && !setup && !githubStatus && <div className="environment-details"><button className="wizard-back" onClick={() => { setKind(undefined); setError(""); }}>← Choose another source</button><div className="wizard-boundary"><GitBranch size={18} /><p><b>Discover from GitHub without installing an agent</b><span>Choose all repositories or specific repositories in GitHub. Lens reads repository metadata and detector-relevant configuration files, then starts discovery automatically.</span></p></div><div className="setup-read"><div><h3>Lens will read</h3><span><CheckCircle2 size={14} />Selected repository metadata</span><span><CheckCircle2 size={14} />Detector-relevant configuration files</span></div><div><h3>Lens will not read</h3><span><X size={14} />Secret values</span><span><X size={14} />Prompts or model inputs and outputs</span><span><X size={14} />Write or repository administration access</span></div></div>{error && <InlineError text={error} />}<button className="button primary full" disabled={busy} onClick={createSetup}>{busy ? "Preparing…" : "Continue to GitHub"}</button></div>}
...
function GitHubDiscoveryProgress({ status, busy, onRetry, onReauthorize, onResults, onAddDevices }: { status: GitHubDiscoveryStatus; busy: boolean; onRetry: () => void; onReauthorize: () => void; onResults: () => void; onAddDevices: () => void }) {
  ...
  const copy = status.phase === "authorizing" ? "Complete authorization in GitHub to start discovery."
    ...
    : status.phase === "scanning" ? `Scanning ${status.selected_repositories} selected repositories. Results appear as each repository completes.`
    : status.phase === "partial" ? "Useful results are ready. Some repositories could not be scanned; retrying will preserve the results already found."
    : status.phase === "ready" ? "Repository discovery is complete. These assets are now part of the same Lens inventory as endpoint evidence."
    : status.safe_error?.message || "GitHub access needs attention before discovery can continue.";
  return <div className={`setup-result github-discovery ${status.phase}`}>
    <div className="scan-progress"><span className="scan-spinner"><GitBranch size={18} /></span><div><p className="eyebrow">GITHUB DISCOVERY</p><h3>{pretty(status.phase)}</h3><p>{copy}</p></div></div>
    {status.selected_repositories > 0 && <div><p>{status.progress.complete + status.progress.failed} of {status.selected_repositories} repositories processed · {status.summary.assets_found} assets found</p><progress max="100" value={status.progress.percent}>{status.progress.percent}%</progress></div>}
    ...
    {(recoverable || status.phase === "awaiting_selection") && <button className="button primary full" disabled={busy} onClick={status.phase === "revoked" ? onReauthorize : onRetry}>{busy ? "Preparing…" : status.phase === "revoked" ? "Authorize GitHub again" : "Retry discovery"}</button>}
    {finished && <><button className="button primary full" onClick={onResults}>View discovered assets <ArrowRight size={15} /></button><button className="button subtle full" onClick={onAddDevices}>Add employee devices</button></>}
  </div>;
}
...
function EnvironmentActivationCard({ api, environment, revision, canManage, onResume, onUpgrade, onScan, onDisconnect }: { api: API; environment: Environment; revision: number; canManage: boolean; onResume: () => void; onUpgrade: () => void; onScan: () => void; onDisconnect: () => void }) {
  ...
  const message = remote.data?.safe_error?.message || environment.last_error_message || quickSummary || (phase === "awaiting_install" ? github ? "GitHub authorization has not completed" : "Installation has not completed" : phase === "connected" ? github ? "Repository connected; awaiting the first result" : "Collector connected; awaiting the first snapshot" : phase === "processing" ? "First results are being normalized; you can leave this page" : phase === "stale" ? `Retained results are visible, but this ${github ? "repository" : "endpoint"} last reported ${relative(remote.data?.last_seen_at || lastResult || "")}` : lastResult ? `Last result ${relative(lastResult)}${phase === "partial" ? " · partial coverage" : ""}` : `Waiting for ${github ? "repository" : "endpoint"} evidence`);
  return <article className="environment-card"><span className="environment-icon"><Icon size={20} /></span><div className="environment-card-copy"><span><b>{environment.display_name}</b><small>{catalog?.title ?? pretty(environment.kind)}{quick ? " · Quick Scan" : ""}{environment.external_id ? ` · ${environment.external_id}` : ""}</small></span><p>{message}</p></div><span className={`connection-status ${phase}`}><i />{pretty(phase)}</span><div className="environment-actions">{canManage && ((environment.kind === "endpoint" && phase === "awaiting_install") || (github && ["setup_pending", "verifying", "auth_error"].includes(environment.connection_status))) && <button className="button subtle" onClick={onResume}>Resume setup</button>}{canManage && remote.data?.can_enable_continuous_monitoring && <button className="button subtle" onClick={onUpgrade}>Enable continuous monitoring</button>}{canManage && environment.connection_status === "connected" && ["aws", "azure", "gcp"].includes(environment.provider || "") && <button className="button subtle" onClick={onScan}><RefreshCw size={14} /> Scan now</button>}{canManage && phase !== "disconnected" && <button className="button quiet" onClick={onDisconnect}>Disconnect</button>}</div></article>;
}
