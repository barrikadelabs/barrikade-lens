...
function EnvironmentsPage({ api, revision, onResults, startWizard = false }: { api: API; revision: number; onResults: () => void; startWizard?: boolean }) {
  ...
  return <div className="page-stack environments-page">
    <section className="panel environment-summary"><div><p className="eyebrow">CONNECTED LOCATIONS</p><h2>{environments.data.items.filter((item) => item.connection_status === "connected").length} connected locations</h2><p>Add the devices, repositories, cloud accounts, and clusters you want Lens to check. Lens never changes them.</p></div>{canManage && <button className="button primary" onClick={() => navigate("/connections/new")}><Plus size={16} /> Add a connection</button>}</section>
...
function CoverageCard({ item, onClick, active }: { item: Overview["coverage"][number]; onClick?: () => void; active?: boolean }) {
  ...
  const status = item.reporting === 0 ? "Not reporting" : [item.fresh ? `${item.fresh} up to date` : "", item.stale ? `${item.stale} not reporting recently` : "", item.partial ? `${item.partial} missing some data` : ""].filter(Boolean).join(" · ");
