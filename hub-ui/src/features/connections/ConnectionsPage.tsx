...
import { ArrowRight, CheckCircle2, ChevronDown, ChevronRight, CircleDot, Cloud, Container, Copy, GitBranch, Monitor, Plus, RefreshCw, ShieldCheck, SlidersHorizontal, X } from "lucide-react";
...
function EnvironmentsPage({ api, revision, onResults, startWizard = false }: { api: API; revision: number; onResults: () => void; startWizard?: boolean }) {
  ...
  }, [api, collectorConnected, environments, navigate, scan, setup]);
  ...
  const reset = () => { setWizard(false); setKind(undefined); setDeploymentMethod(undefined); setName(""); setExternalID(""); setTenantID(""); setProjectNumber(""); setSetup(undefined); setScan(undefined); setCollectorConnected(false); setMessage(""); setError(""); setHandoff(undefined); if (location.pathname !== "/connections") navigate("/connections", { replace: true }); };
  const createSetup = () => {
    if (!kind) return;
    setBusy(true); setError("");
    const configuration = kind === "azure_subscription" ? { tenant_id: tenantID } : kind === "gcp_project" ? { project_number: projectNumber } : kind === "endpoint" ? { deployment_method: deploymentMethod, platform } : {};
  ...
  return <div className="page-stack environments-page">
    ...
    {wizard && canManage && <div className="modal-overlay environment-wizard-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) reset(); }}><section className="environment-wizard" role="dialog" aria-modal="true" aria-label="Scan environment"><button className="drawer-close" aria-label="Close scan setup" onClick={reset}><X size={18} /></button><header><p className="eyebrow">SCAN ENVIRONMENT</p><h2>{setup ? "Complete source setup" : kind === "endpoint" && !deploymentMethod ? "How do you want to deploy?" : kind ? `Connect ${selected?.title}` : "Let’s map your AI environment"}</h2><p>{setup ? "The generated setup is least-privilege and expires shortly. Lens stores no long-lived cloud keys." : kind === "endpoint" ? "Choose a deployment method before Lens shows platform-specific instructions." : "Connect the places where AI might be running."}</p></header>
      ...
      {kind === "endpoint" && !setup && !deploymentMethod && <div className="environment-details"><button className="wizard-back" onClick={() => setKind(undefined)}>← Choose another source</button><EmployeeDeploymentMethods onSelect={(method) => { setDeploymentMethod(method); captureAnalytics({ name: "deployment_method_selected", properties: { deployment_method: method } }); }} /></div>}
      {kind === "endpoint" && !setup && deploymentMethod && <div className="environment-details"><button className="wizard-back" onClick={() => { setDeploymentMethod(undefined); setError(""); }}>← Choose another deployment method</button><div className="platform-step"><h3>Choose a platform</h3><p>Lens uses the device hostname, platform, architecture, and installation identity after enrollment. You can rename the device later.</p></div><div className="platform-picker" aria-label="Installation platform">{(["macos", "windows", "linux"] as const).map((value) => <button key={value} aria-pressed={platform === value} className={platform === value ? "active" : ""} onClick={() => setPlatform(value)}>{value === "macos" ? "macOS" : pretty(value)}</button>)}</div><div className="wizard-boundary"><ShieldCheck size={18} /><p><b>Read-only by design</b><span>Lens inventories software, runtime state, network listeners, and configuration references. It excludes prompts, outputs, secrets, and file contents.</span></p></div>{error && <InlineError text={error} />}<button className="button primary full" disabled={busy} onClick={createSetup}>{busy ? "Preparing…" : "Start scan"}</button></div>}
